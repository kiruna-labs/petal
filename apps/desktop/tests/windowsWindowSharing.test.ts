...
test('Windows receiver forwards URL metadata on subscribe and refresh', () => {
  ...
  // 4000, not 3000: the arm also carries the metadata teardown fallback added
  // with the shared-window video delivery work (a sharer that clears its title
  // map closes the stale window without waiting for TrackUnpublished), which
  // sits between this arm's start and its URL decode.
  assert.match(
    subscriber,
    /ParticipantMetadataChanged[\s\S]{0,4000}shared_window_url_from_metadata/,
