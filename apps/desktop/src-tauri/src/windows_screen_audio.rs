...
use windows::Win32::System::Com::{
    CoInitializeEx, CoTaskMemAlloc, CoTaskMemFree, CoUninitialize, BLOB, COINIT_MULTITHREADED,
};
...
fn open_audio_client(
    ...
    match source {
        AudioSourceKey::SystemOutput => {
            // Feedback safety is a hard availability requirement. Ordinary
            // endpoint loopback can recapture Petal's own meeting playout, so
            // system audio is video-only unless Windows can exclude Petal's
            // complete process tree. Never broaden to default-render fallback.
            if !process_loopback_supported() {
                return Err(format!(
                    "safe system audio requires Windows build {PROCESS_LOOPBACK_MIN_BUILD} or newer"
                ));
            }
            ...
            .map_err(|error| {
                format!("safe system audio unavailable because Petal exclusion failed: {error}")
            })?;
            Ok((client, None, true))
        }
