...
pub mod native {
    ...
    // PETAL PATCH (#787): the severity mapping the installed native log sink
    // uses, re-exported so the app crate can assert against the real function
    // rather than a copy of it. See PETAL_PATCH.md.
    pub use crate::imp::peer_connection_factory::webrtc_log_level;
}
