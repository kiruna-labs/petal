...
/// PETAL PATCH (#787): map native WebRTC severity onto a `log` level.
///
/// Upstream discarded the severity and logged every native line at `debug`,
/// so an `RTC_LOG(LS_ERROR)` from the vendored C++ -- including the ADM
/// playout failures in `webrtc-sys/src/adm_proxy.cpp`, the only evidence a
/// silent meeting leaves behind -- could not reach `petal.log` at any default
/// filter, and never became a Sentry event. `Info`/`Verbose` stay below the
/// app's default filter on purpose: native WebRTC emits those per packet on
/// some paths.
pub fn webrtc_log_level(severity: sys_rtc::ffi::LoggingSeverity) -> log::Level {
    match severity {
...
/// The registered sink callback. A named `fn` rather than a closure so the
/// exact code path the sink runs is directly callable from a test.
fn emit_webrtc_log(msg: String, severity: sys_rtc::ffi::LoggingSeverity) {
    let msg = msg.strip_suffix("\r\n").or(msg.strip_suffix('\n')).unwrap_or(&msg);
    log::log!(target: "libwebrtc", webrtc_log_level(severity), "{}", msg);
}
...
impl Default for PeerConnectionFactory {
    fn default() -> Self {
        ...
        if log_sink.is_none() {
            *log_sink = Some(sys_rtc::ffi::new_log_sink(emit_webrtc_log));
        }
