...
//
// PETAL PATCH (see apps/desktop/vendor/screencapturekit/PETAL_PATCH.md):
// renamed from `AudioBufferBridge`/`AudioBufferListRaw` to
// `SCKAudioBufferBridge`/`SCKAudioBufferListRaw`. apple-cf 0.9.3's own
// CoreMediaBridge Swift module declares identically-named public structs
// with the same layout; both modules are literally named `CoreMediaBridge`
// (see each crate's Package.swift), so once `-ObjC` forces whole-archive
// loading (required by livekit/webrtc-sys on macOS), the linker sees two
// definitions of the same Swift type metadata symbol and fails with
// `duplicate symbol` errors. The two `@_cdecl` C functions that use these
// structs (below) keep their exact names and ABI — Rust only crosses this
// FFI boundary through those C symbols and matching `#[repr(C)]` structs on
// the Rust side (see `src/cm/audio.rs`/`src/cm/ffi.rs`), never through the
// Swift type name — so this rename is safe and does not change behavior.
...
public func cm_sample_buffer_get_frame_status(_ sampleBuffer: UnsafeMutableRawPointer) -> Int32 {
    ...
    // The status attachment is a CFNumber (NSNumber), NOT a boxed SCFrameStatus
    // enum -- casting `as? SCFrameStatus` always fails and returns -1 (frame
    // status silently unavailable). Read it as an NSNumber and return the raw
    // Int32; the Rust side maps it via SCFrameStatus::from_raw.
    guard let attachments = CMSampleBufferGetSampleAttachmentsArray(buffer, createIfNecessary: false) as? [[CFString: Any]],
          let firstAttachment = attachments.first,
          let statusNumber = firstAttachment[SCStreamFrameInfo.status.rawValue as CFString] as? NSNumber
    else {
        return -1
    }

    return statusNumber.int32Value
}
