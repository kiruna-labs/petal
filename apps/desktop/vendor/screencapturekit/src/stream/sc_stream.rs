...
impl SCStream {
    ...
    pub fn new(filter: &SCContentFilter, configuration: &SCStreamConfiguration) -> Self {
        ...
        Self {
            inner: Arc::new(SCStreamInner { ptr, context }),
        }
    }
    ...
    pub fn add_output_handler_with_queue(
        ...
        let ok = if let Some(q) = queue {
            ...
            unsafe { ffi::sc_stream_add_stream_output(self.inner.ptr, output_type_int) }
        };

        if ok {
            // SAFETY: inner.context is the Box::into_raw StreamContext created in
            // SCStream::new; it stays valid until the final SCStream owner drops.
    ...
    pub fn remove_output_handler(&mut self, id: usize, of_type: SCStreamOutputType) -> bool {
        // SAFETY: inner.context is the Box::into_raw StreamContext created in
        // SCStream::new; it stays valid until the final SCStream owner drops.
        let mut handlers = unsafe { &*self.inner.context }
            .handlers
            .write()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        ...
        if !has_type {
            ...
            unsafe { ffi::sc_stream_remove_stream_output(self.inner.ptr, output_type_int) };
        }
    ...
    pub fn start_capture(&self) -> Result<(), SCError> {
        ...
        unsafe { ffi::sc_stream_start_capture(self.inner.ptr, context, UnitCompletion::callback) };
    ...
    pub fn stop_capture(&self) -> Result<(), SCError> {
        let (completion, context) = UnitCompletion::new();
        unsafe { ffi::sc_stream_stop_capture(self.inner.ptr, context, UnitCompletion::callback) };
    ...
    pub fn synchronization_clock(&self) -> Option<crate::cm::CMClock> {
        let ptr = unsafe { ffi::sc_stream_get_synchronization_clock(self.inner.ptr) };
...
mod tests {
    ...
    struct DropTrackingDelegate(Arc<AtomicUsize>);

    impl SCStreamDelegateTrait for DropTrackingDelegate {}

    impl Drop for DropTrackingDelegate {
        fn drop(&mut self) {
            self.0.fetch_add(1, Ordering::Relaxed);
        }
    }

    /// A cloned configuration handle must not tear down the callback state
    /// shared by the live stream. Only the final SCStream owner may release it.
    #[test]
    fn test_clone_releases_callback_state_only_after_final_owner() {
        let delegate_drops = Arc::new(AtomicUsize::new(0));
        let context = StreamContext::new_with_delegate(Box::new(DropTrackingDelegate(
            delegate_drops.clone(),
        )));
        let stream = SCStream {
            // A null stream pointer keeps this ownership-only test independent
            // of a live ScreenCaptureKit session; final Drop still releases the
            // real StreamContext below.
            inner: Arc::new(SCStreamInner {
                ptr: std::ptr::null(),
                context,
            }),
        };
        let inner_lifetime = Arc::downgrade(&stream.inner);
        let temporary_configuration_handle = stream.clone();

        drop(temporary_configuration_handle);
        assert!(
            inner_lifetime.upgrade().is_some(),
            "dropping one clone released shared stream ownership"
        );
        assert_eq!(
            delegate_drops.load(Ordering::Relaxed),
            0,
            "dropping one clone destroyed the live callback delegate"
        );

        drop(stream);
        assert!(
            inner_lifetime.upgrade().is_none(),
            "final owner did not release shared stream ownership"
        );
        assert_eq!(
            delegate_drops.load(Ordering::Relaxed),
            1,
            "final owner must destroy callback state exactly once"
        );
    }
