...
fn main() {
    ...
    // Resolve the Windows SDK's lib directory (containing mfuuid.lib, which
    // is a static import lib rustc must find via -L). Prefers the
    // WindowsSdkDir environment variable, falling back to the standard
    // install location; uses the highest 10.* SDK version found.
    fn windows_sdk_lib_dir(target_arch: &str) -> Option<PathBuf> {
        ...
        let roots: Vec<PathBuf> = {
            ...
            let pf = env::var("ProgramFiles(x86)")
                ...
                .unwrap_or_default();
            if !pf.is_empty() {
                roots.push(PathBuf::from(pf).join("Windows Kits").join("10"));
            }
            roots
        };
        for root in roots {
            ...
            if let Ok(entries) = std::fs::read_dir(&lib) {
                ...
                for version in versions {
                    let candidate = lib.join(version).join("um").join(arch_dir);
                    if candidate.join("mfuuid.lib").exists() {
    ...
    match target_os.as_str() {
        "windows" => {
            ...
            // mfuuid is a static import lib shipped with the Windows SDK
            // (not a DLL); rustc -l static: only searches -L paths, not the
            // LIB env, so emit the resolved SDK lib dir.
            println!("cargo:rustc-link-lib=static=mfuuid");
            if let Some(sdk_lib_dir) = windows_sdk_lib_dir(&target_arch) {
                println!("cargo:rustc-link-search=native={}",
                         sdk_lib_dir.to_string_lossy());
            }
