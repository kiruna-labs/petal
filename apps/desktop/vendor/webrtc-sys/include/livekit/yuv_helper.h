...
// libyuv I420Scale (bilinear) with a destination region pointer: the caller
// may pass a dst pointer offset inside a larger buffer (letterbox padding) as
// long as dst_stride is the FULL buffer stride.
static void i420_scale(const uint8_t* src_y,
                       int src_stride_y,
                       const uint8_t* src_u,
                       int src_stride_u,
                       const uint8_t* src_v,
                       ...
                       int dst_width,
                       int dst_height) {
  THROW_ON_ERROR(libyuv::I420Scale(src_y, src_stride_y, src_u, src_stride_u,
