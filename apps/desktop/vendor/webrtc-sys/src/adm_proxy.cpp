...
void AdmProxy::SwitchPlayoutModeIfNeeded() {
  ...
  if (use_platform) {
    ...
    // #787: these return codes were ignored, so a CoreAudio failure here
    // would leave playout on nothing at all -- no log, no retry anywhere
    // above. Check, retry once, and scream on failure.
    if (synthetic_adm_) {
      synthetic_adm_->StopPlayout();
    }
    if (platform_adm_) {
      int32_t init = platform_adm_->InitPlayout();
      int32_t start = platform_adm_->StartPlayout();
      if (init != 0 || start != 0) {
        RTC_LOG(LS_WARNING) << "AdmProxy: switch to platform playout failed (init="
                            << init << " start=" << start << ") -- retrying once (#787)";
        init = platform_adm_->InitPlayout();
        start = platform_adm_->StartPlayout();
      }
      if (init != 0 || start != 0) {
        RTC_LOG(LS_ERROR) << "AdmProxy: platform playout could not be started (init="
                          << init << " start=" << start
                          << ") -- remote audio will be INAUDIBLE until playout restarts (#787)";
      }
    }
  } else {
...
void AdmProxy::SwitchRecordingAdmIfNeeded() {
  ...
  if (adm) {
    int32_t init = adm->InitRecording();
    int32_t start = adm->StartRecording();
    if (init != 0 || start != 0) {
      RTC_LOG(LS_ERROR) << "AdmProxy: recording ADM switch failed (init=" << init
                        << " start=" << start << ") -- microphone will capture NOTHING";
    }
  } else {
    recording_ = false;
  }
...
int32_t AdmProxy::StartPlayout() {
  ...
  if (is_platform_playout_active()) {
    if (platform_adm_) {
      // #787: InitPlayout() and StartPlayout() each evaluate the active mode
      // at call time, so a playout enable landing between them would hand
      // the platform ADM a StartPlayout without init -> permanent silence.
      // Structurally possible (never reproduced live); closed here.
      if (!platform_adm_->PlayoutIsInitialized()) {
        int32_t init = platform_adm_->InitPlayout();
        if (init == 0) {
          playout_initialized_ = true;
        } else {
          RTC_LOG(LS_ERROR) << "AdmProxy::StartPlayout() - platform InitPlayout failed error="
                            << init << " (#787)";
        }
      }
      int32_t result = platform_adm_->StartPlayout();
      if (result != 0) {
        RTC_LOG(LS_ERROR) << "AdmProxy::StartPlayout() - platform StartPlayout failed error="
                          << result << " -- remote audio will be INAUDIBLE (#787)";
      }
      return result;
    }
    return -1;
...
int32_t AdmProxy::StartRecording() {
  ...
  recording_ = true;
  // #787 twin: same split-brain as StartPlayout, mic-silence flavor.
  if (!adm->RecordingIsInitialized()) {
    int32_t init = adm->InitRecording();
    if (init == 0) {
      recording_initialized_ = true;
    } else {
      RTC_LOG(LS_ERROR) << "AdmProxy::StartRecording() - InitRecording failed error=" << init;
    }
  }
  int32_t result = adm->StartRecording();
  if (result != 0) {
    RTC_LOG(LS_ERROR) << "AdmProxy::StartRecording() - StartRecording failed error=" << result;
  }
  return result;
}
