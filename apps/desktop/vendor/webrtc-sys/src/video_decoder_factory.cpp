...
#if defined(RTC_DAV1D_IN_INTERNAL_DECODER_FACTORY)
  // AV1 is only advertised as a RECEIVE codec when the decoder factory can
  // actually create an AV1 decoder. Without dav1d the AV1 branch in Create()
  // is compiled out and an SDP answer claiming AV1 support would produce a
  // nullptr decoder -> black frames (Petal hard rule #627). Keep the
  // advertisement honest: no dav1d, no AV1 in GetSupportedFormats (which also
  // makes QueryCodecSupport say the truth).
