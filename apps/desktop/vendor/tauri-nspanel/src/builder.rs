...
impl<'a, R: Runtime + 'a, T: FromWindow<R> + 'static> PanelBuilder<'a, R, T> {
    ...
    ///
    /// Since PanelBuilder creates a regular window before converting it to a
    /// panel, the window creation phase can steal focus (tao's build performs
    /// a key-and-order-front for a default-visible window). When set to true,
    /// the window is built `.visible(false).focused(false)` instead -- a
    /// per-window mechanism; callers show the panel themselves afterwards.
    ///
    /// Petal patch (#824): this REPLACES the upstream implementation, which
    /// temporarily flipped the app's activation policy to Prohibited around
    /// the build. Creating windows under Prohibited violates AppKit's own
    /// contract for that policy, and -- measured live on macOS 26.5 -- every
    /// such flip made the Dock register a DUPLICATE application tile (three
    /// startup panels = three "Petal" Dock icons for one process). An
    /// explicit `.visible(true)` from `with_window` still wins: the caller
    /// override runs after this default.
    ///
    /// # Example
    /// ```rust
    /// use tauri_nspanel::{PanelBuilder, PanelLevel};
    ...
    pub fn build(self) -> tauri::Result<Arc<dyn Panel<R>>> {
        // Petal patch (#824): `no_activate` is implemented per-window --
        // `.visible(false).focused(false)` below -- NOT via the upstream
        // process-global activation-policy flip, which is removed. Measured
        // live on macOS 26.5: each Prohibited->Regular flip made the Dock
        // register a DUPLICATE application tile (three startup panels ->
        // three "Petal" icons for one process; flip disabled -> exactly one
        // tile, cleanly removed on quit). The flip also violated AppKit's
        // Prohibited contract ("may not create windows"), raced
        // PETAL_ACCESSORY_UI's Accessory policy, and its restore path
        // already cost one leak fix (#705) -- that whole class is deleted
        // with it. See PETAL_PATCH.md.
        ...
        // #824: the no_activate mechanism. Applied BEFORE `window_fn` so an
        // explicit caller `.visible(true)` still wins. Every no_activate
        // caller hides/reveals its panel itself, so none relies on
        // build-time visibility -- and a hidden build cannot key-and-order-
        // front, which is the whole focus-steal (#677 class) this prevents.
        if self.panel_config.no_activate.unwrap_or(false) {
            window_builder = window_builder.visible(false).focused(false);
        }
...
#[cfg(test)]
mod petal_no_activate_mechanism_tests {
    /// Source-shape guard (#824, same pattern as the #675/#677 pins): the
    /// `no_activate` option MUST be implemented as a hidden+unfocused window
    /// build, and the process-global activation-policy flip must never come
    /// back -- each Prohibited->Regular flip mints a duplicate Dock tile for
    /// the app on macOS 26 (measured live: three startup panels = three
    /// "Petal" Dock icons), and creating windows under Prohibited violates
    /// AppKit's contract for that policy. A future re-vendor that restores
    /// upstream's dance fails here.
    #[test]
    fn no_activate_builds_hidden_and_unfocused_never_flips_activation_policy() {
        let body = include_str!("builder.rs");
        assert!(
            body.contains("window_builder = window_builder.visible(false).focused(false);"),
            "no_activate must be implemented as a hidden+unfocused window build (#824)"
        );
        let build_fn_start = body
            .find("pub fn build(self) -> tauri::Result<Arc<dyn Panel<R>>>")
            .expect("PanelBuilder::build must exist");
        let tests_start = body
            .find("mod petal_no_activate_mechanism_tests")
            .expect("this test module must exist");
        // Scan only real code: this test module's own strings contain the
        // forbidden identifier (the self-matching-pattern trap, pgrep-style).
        let build_fn = &body[build_fn_start..tests_start];
        assert!(
            !build_fn.contains("setActivationPolicy"),
            "build() must never flip the app activation policy again -- the \
             Prohibited dance duplicates the Dock tile per panel (#824)"
        );
    }
}
