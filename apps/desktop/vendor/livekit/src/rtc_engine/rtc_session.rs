...
impl SessionInner {
                ...
                Some(event) = rtc_events.recv() => {
                    ...
                    tokio::select! {
                        ...
                        _ = livekit_runtime::sleep(Duration::from_secs(10)) => {
                            log::warn!("rtc_event is taking too much time: {}", debug);
                        }
                    }
                    ...
                    match signal {
                        SignalEvent::Message(signal) => {
                            ...
                            tokio::select! {
                                _ = rx => {},
                                _ = livekit_runtime::sleep(Duration::from_secs(10)) => {
                                    log::warn!("signal_event taking too much time: {}", debug);
                                }
                            }
    ...
    async fn create_sender(
        ...
        if track.kind() == TrackKind::Video {
            // Petal patch: a native screen share asks the encoder for detail
            // and for resolution over frame rate when a sender is constrained,
            // which is what the web sender does. Cameras keep the defaults.
            ...
            let matched = options::preferred_video_codecs(
                capabilities.codecs,
                options.video_codec,
