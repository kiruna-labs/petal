...
impl RtcEngine {
    ...
    pub fn remove_track(&self, sender: RtpSender) -> EngineResult<()> {
        // Keep close() from tearing down the native peer connection between the
        // closed-state check and the synchronous CXX remove_track call.
        let running_handle = self.inner.running_handle.read();
        if running_handle.closed {
            return Err(EngineError::Connection("engine is closed".into()));
        }
        running_handle.session.remove_track(sender) // TODO(theomonnom): Ignore errors where this
...
impl EngineInner {
    ...
    async fn wait_reconnection(
        ...
        RwLockReadGuard<'_, EngineHandle>,
        AsyncRwLockReadGuard<'_, ()>,
    )> {
    ...
    async fn reconnect_task(self: &Arc<Self>) -> EngineResult<()> {
        ...
        for i in 1..=RECONNECT_ATTEMPTS {
            ...
            if full_reconnect {
                ...
                log::warn!("restarting connection... attempt: {}", i);
                match self
                ...
                log::warn!("resuming connection... attempt: {}", i);
