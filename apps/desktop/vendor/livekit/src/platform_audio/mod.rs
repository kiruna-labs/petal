...
impl PlatformAudio {
    ...
    ///
    /// [`start_recording`]: Self::start_recording
    /// Force the ADM proxy to re-run its synthetic-vs-platform playout mode
    /// switch (#787). `set_adm_playout_enabled` early-returns on an unchanged
    /// value, so a failed platform `InitPlayout`/`StartPlayout` inside the
    /// switch was otherwise permanently unreachable from Rust. The toggle
    /// re-drives the full Init+Start pair; on a healthy call it costs one
    /// brief playout restart.
    pub fn reassert_playout(&self) {
        self.handle.runtime.set_adm_playout_enabled(false);
        self.handle.runtime.set_adm_playout_enabled(true);
        log::info!("PlatformAudio: re-asserted ADM playout mode (#787)");
    }
