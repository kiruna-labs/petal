...
/// Per-layer sender parameters that can be changed without unpublishing the
/// track. The RID and all other RTP encoding fields remain owned by the SDK;
/// callers may only tune the two screencast controls that WebRTC permits to be
/// changed while an RTP sender is live.
#[derive(Clone, Debug, PartialEq)]
pub struct PublishingLayerParameters {
    pub rid: String,
    pub max_bitrate: u64,
...
impl LocalVideoTrack {
    ...
    /// Read back the sender's current per-layer bitrate and frame-rate caps.
    /// This is a sender-parameter readback, not a promise that a hardware
    /// encoder accepted every backend-specific knob.
    pub fn publishing_layer_parameters(&self) -> Vec<PublishingLayerParameters> {
        let Some(transceiver) = self.transceiver() else {
            return Vec::new();
        };
        transceiver
            ...
            .encodings
            .into_iter()
            .map(|encoding| PublishingLayerParameters {
                rid: encoding.rid,
    ...
    /// Update per-layer bitrate and frame-rate limits on the live RTP sender.
    ///
    /// This is the public application-facing counterpart to the internal
    /// dynacast path. It deliberately does not add/remove encodings or change
    /// their scale/RID, so layout changes still require a republish.
    pub fn set_publishing_layer_parameters(
        &self,
        updates: &[PublishingLayerParameters],
    ) -> RoomResult<()> {
        let transceiver = self.transceiver().ok_or_else(|| {
            RoomError::Internal("cannot set publishing parameters: no transceiver".into())
        })?;
        ...
        if params.encodings.is_empty() {
            return Err(RoomError::Internal(
                "cannot set publishing parameters: sender has no encodings".into(),
            ));
        }

        let mut changed = false;
        for encoding in &mut params.encodings {
            let Some(update) = updates.iter().find(|update| update.rid == encoding.rid) else {
                continue;
            };
            let max_bitrate = Some(update.max_bitrate);
            let max_framerate = Some(update.max_framerate);
            if encoding.max_bitrate != max_bitrate || encoding.max_framerate != max_framerate {
