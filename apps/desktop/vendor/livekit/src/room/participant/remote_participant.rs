    ...
    pub(crate) fn add_publication(&self, publication: TrackPublication) {
        ...
        publication.on_enabled_status_changed({
            ...
            move |publication, enabled| {
                let rtc_engine = rtc_engine.clone();
                // Petal patch (#907): send the publication's own last
                // explicitly-requested quality instead of hard-coding High,
                // so an enabled-state flip cannot silently undo a receiver
                // starvation downgrade.
                let quality: i32 = proto::VideoQuality::from(publication.desired_video_quality()).into();
                livekit_runtime::spawn(async move {
                    let tsid: String = publication.sid().into();
        ...
        // Keep this state beside the only dimension-variant sender. The
        // receiver may repeat its steady-state hint on a heartbeat; that must
        // not create another UpdateTrackSettings request.
        let last_sent_dimensions = Arc::new(Mutex::new(None));
        publication.on_video_dimensions_changed({
            let rtc_engine = self.inner.rtc_engine.clone();
            let last_sent_dimensions = last_sent_dimensions.clone();
            move |publication, dimension| {
                let mut last_sent = last_sent_dimensions.lock();
                ...
                *last_sent = Some(dimension);
                ...
                let rtc_engine = rtc_engine.clone();
                // Petal patch (#907): same fix as `on_enabled_status_changed`
                // above -- send the last explicitly-requested quality, not a
                // hard-coded High, so a viewer resize (which fires this
                // dimension-changed callback) cannot silently re-upgrade a
                // subscription a starvation guard just downgraded to Low.
                let desired_quality = publication.desired_video_quality();
                let quality: i32 = proto::VideoQuality::from(desired_quality).into();
                livekit_runtime::spawn(async move {
                    let tsid: String = publication.sid().into();
                    ...
                    log::info!(
                        "subscriber: sending UpdateTrackSettings to SFU: track={}, dimensions={}x{}, quality={:?}, disabled={}",
                        tsid,
                        width,
                        height,
                        desired_quality,
