...
struct RemoteInfo {
    subscribed: bool,
    allowed: bool,
    last_sent_video_dimensions: Mutex<Option<TrackDimension>>,
}

struct RemoteInner {
    info: RwLock<RemoteInfo>,
    events: RemoteEvents,
    // Petal patch (#907): the SFU-desired simulcast quality this publication
    // last explicitly asked for via `set_video_quality`. `on_video_dimensions_changed`
    // and `on_enabled_status_changed` below read this instead of hard-coding
    // `High`, so a receiver-driven quality downgrade (bandwidth starvation
    // recovery) survives an unrelated dimension/enabled-state update instead
    // of being silently clobbered back to High. Defaults to `High` to
    // preserve pre-patch behavior for any publication that never calls
    // `set_video_quality`.
    desired_video_quality: Mutex<VideoQuality>,
}
...
impl RemoteTrackPublication {
        ...
        Self {
            ...
            remote: Arc::new(RemoteInner {
                info: RwLock::new(RemoteInfo {
                    ...
                    allowed: true,
                    last_sent_video_dimensions: Mutex::new(None),
                }),
                events: Default::default(),
                desired_video_quality: Mutex::new(VideoQuality::High),
            }),
        }
    }
    ...
    pub fn set_video_quality(&self, quality: VideoQuality) {
        ...
        // Petal patch (#907): record BEFORE firing the callback so that any
        // dimension/enabled-state update racing this call (even one already
        // queued) observes the new desired quality rather than a stale one.
        *self.remote.desired_video_quality.lock() = quality;
    ...
    /// Petal patch (#907): the quality last explicitly requested via
    /// `set_video_quality`, or `High` if that has never been called. Read by
    /// the dimension/enabled-state `UpdateTrackSettings` senders in
    /// `RemoteParticipant::add_publication` so every subscription-setting
    /// request this publication sends agrees on one desired quality, instead
    /// of the dimension/enabled paths hard-coding `High` independently of
    /// whatever `set_video_quality` most recently asked for.
    pub(crate) fn desired_video_quality(&self) -> VideoQuality {
        *self.remote.desired_video_quality.lock()
    }
    ...
    pub fn update_video_dimensions(&self, dimension: TrackDimension) {
        if self.is_subscribed() {
            // This is a receiver-side subscription hint, not a publisher
            // TrackInfo update. Keeping it out of `inner.info` preserves
            // `dimension()` as the publisher's canonical encode resolution.
            if let Some(video_dimensions_changed) =
                self.remote.events.video_dimensions_changed.lock().as_ref()
            {
                {
                    let info = self.remote.info.read();
                    let mut last_sent = info.last_sent_video_dimensions.lock();
                    if *last_sent == Some(dimension) {
                        return;
                    }
                    *last_sent = Some(dimension);
                }
...
mod tests {
    ...
    #[test]
    fn steady_dimension_hints_do_not_reach_the_sender_twice() {
        ...
        // The viewer-demand heartbeat reaches this production callback with
        // the same dimensions while a remote window is steady.
        publication.update_video_dimensions(TrackDimension(1280, 720));
        publication.update_video_dimensions(TrackDimension(1280, 720));

        assert_eq!(sends.load(Ordering::Relaxed), 1);
        assert_eq!(publication.dimension(), TrackDimension(1920, 1080));
    }
}
