...
impl Room {
    ...
    pub async fn connect(
        ...
        e2ee_manager.on_state_changed({
            let dispatcher = dispatcher.clone();
            // PETAL PATCH (#883): capture Weak, not Arc. RoomSession owns
            // e2ee_manager, which stores this closure -- a strong capture is
            // a self-cycle that kept EVERY RoomSession (and its engine,
            // session, and both peer connections) alive forever: ~320KB of
            // still-reachable memory per Room connect/close, measured linear
            // to 1000 cycles. Mirrors the `set_session(Arc::downgrade(..))`
            // pattern used three lines above for exactly this reason.
            ...
            move |participant_identity, state| {
                ...
                let Some(inner) = inner.upgrade() else {
                    // Room already torn down; nothing to dispatch to.
                    return;
                };
...
impl RoomSession {
    ...
    async fn close(&self, reason: DisconnectReason) -> RoomResult<()> {
        ...
        // A server disconnect has already torn down the peer connection, so attempting to
        // remove its senders can cross the CXX bridge with an invalid native handle.
        if self.info.read().state != ConnectionState::Disconnected {
