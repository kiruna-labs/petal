...
| `PETAL_CAMERA_SYNTH_FPS` | desktop synthetic camera source | Raises the synthetic chart to a fixed 1280×720@60 for the rate-control resolution comparison. Accepts exactly `60`; any other value (and an unset variable) keeps the 30 fps default. Every frame still advances the sentinel, so the higher rate is never implemented by repeating a frame. **Only honored together with `PETAL_CAMERA_SYNTH_SOURCE=1`**. |
...
| `PETAL_CAMERA_DEGRADATION_PREFERENCE` | desktop camera publish | Camera degradation policy. Diagnostic arm only; unset and every unrecognized value stay WebRTC-native. `maintain-resolution` holds the source resolution, which was MEASURED WORSE on the Windows-to-macOS route: the send-side estimate opens near 0.7 Mbps, so a held 720p encoder is starved to ~700 kbps (QP 37-40) and stalls far longer than native resolution adaptation (14 freezes / 12683 ms vs 5 / 3780 ms in the first 16 s). Do not promote without a matched run that says otherwise. |
