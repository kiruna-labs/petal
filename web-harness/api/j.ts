...
// Phones and tablets get one quiet link here instead of download buttons: a
// .dmg or .exe is no use on the device showing the invite (#242).
const DESKTOP_APPS_URL = 'https://petal.live/docs/getting-started/install/';
...
// Phones and tablets cannot run the desktop app, so they get "Join in browser"
// as the only action and no petal:// hand-off (#242). Same user-agent rule as
// the browser client's isPhoneOrTablet (#243). iPadOS Safari sends a Mac user
// agent, which only the page can tell apart (by touch points), so the page
// script in inviteInterstitialHtml finishes the job before the hand-off fires.
export function isPhoneOrTabletUserAgent(userAgent: string | undefined): boolean {
...
export function inviteInterstitialHtml(args: {
  ...
  const downloadNotice = downloadPlatform === 'windows'
    ...
    : '';
  const phoneOrTablet = args.phoneOrTablet ?? false;
  // A known phone or tablet gets the mobile layout only: no Open Petal, no
  // downloads, no hand-off script. Everything else gets both layouts, and the
  // class="mobile" that picks between them is set by the iPadOS check.
  const desktopLayout = phoneOrTablet ? '' : `
  ...
  <div class="mobile-only">
    <p class="copy">Nothing to install — join from this browser.</p>
    <div class="actions">
      <a class="button primary" href="${escapeHtml(webJoinUrl)}" rel="noreferrer">Join in browser</a>
    </div>
  </div>`;
  const mobileFooter = `
  <p class="mobile-only desktop-apps">On a computer? <a href="${DESKTOP_APPS_URL}" rel="noreferrer">Download Petal for Windows or macOS</a></p>`;
  // iPadOS Safari reports a desktop "Macintosh" user agent, but no Mac has a
  // touch screen. Runs in <head> so the desktop layout never paints first.
  const tabletCheck = phoneOrTablet ? '' : `
<script>
  if (/Macintosh/i.test(navigator.userAgent) && navigator.maxTouchPoints > 1) {
    document.documentElement.classList.add('mobile');
  }
</script>`;
  const handoff = phoneOrTablet ? '' : `
<script>
  ...
  return `<!doctype html>
<html lang="en"${phoneOrTablet ? ' class="mobile"' : ''}>
<head>
<meta charset="utf-8" />
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Quiet, but a full-size tap target: the padding is cancelled by the
     negative margin, so the line's layout doesn't move. */
  .desktop-apps a {
  ...
  /* Phones in landscape: keep the whole page on one screen. */
  @media (max-height: 440px) {
    .mobile body { padding: 16px 24px; }
    .mobile .brand { margin-bottom: 14px; }
    .mobile h1 { margin-bottom: 8px; font-size: clamp(24px, 7vh, 42px); }
    .mobile .copy { margin-bottom: 18px; font-size: 14px; }
    .mobile .fine { margin-top: 14px; }
    .mobile .desktop-apps { margin-top: 18px; }
  }
</style>${tabletCheck}
</head>
<body>
<main>
  <p class="brand">
    <svg class="brand-mark" width="32" height="32" viewBox="0 0 936 961" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
  ...
  <h1>${escapeHtml(title)}</h1>${desktopLayout}${mobileLayout}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const userAgent = firstQueryValue(req.headers['user-agent']);
  const downloadPlatform = desktopDownloadPlatformForUserAgent(userAgent);
  // Chromium browsers also flag phones directly: Sec-CH-UA-Mobile: ?1.
  const phoneOrTablet = firstQueryValue(req.headers['sec-ch-ua-mobile']) === '?1'
    || isPhoneOrTabletUserAgent(userAgent);
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  // The page differs by device, and Vercel's edge cache ignores Vary.
