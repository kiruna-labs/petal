...
test('connecting registers the room and local identity in the sensitive-string registry before any log line', async () => {
  ...
  try {
    ...
    // guard's history entry) for the whole meeting; #245 also registers the
    // room label's invite-URL slug.
    assert.equal(registry.scrub(`/design-review/${ACCESS_CODE}`), '/<redacted:room>/<redacted:room>');
