...
| Panel (drawer) | the meeting **chat** is the drawer (I-7a, a host surface): a 320 px right column beside the gallery in `MeetingChrome`, covering the gallery under 640 px; "Chat" control with an unread badge in the gallery bar and the pill's More menu; a toast while closed | same column beside the tiles (`.meeting-body`); on a landscape phone (a touch screen at most 500 px tall; a short desktop window keeps its chrome) the top bar and dev tools step out while it is open, so the column runs from the top to the control bar (with the soft keyboard up, at most 300 px tall and 2:1 or wider, the control bar steps out too; a portrait phone's keyboard leaves it narrower, so Leave stays); "Chat" control with badge | `shared/ui/components/ChatDrawer.svelte`, `shared/logic/chat.ts` (wire model, store, history); Rust `chat.rs` transports and stamps; contract `chatMessages`/`chatLimits`/`chatDataEvent`. A generic plugin-owned panel frame is not built; plugins reach the drawer through the `chat` API (I-7b). |
